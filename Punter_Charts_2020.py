...
def football_field(linenumbers=True,
    ...
    ax.annotate("NFL", (cx, cy), color='black', weight='bold', fontsize=20, ha='center', va='center')
...
def plot_passes(punter_names = 'Tommy Townsend'):
  ...
  ax.scatter(puntReceived['ball_y'], puntReceived['kickLength'], color='black', alpha=0.6, edgecolors='white')
  # scatter plot incompletions as small red dots with some transparency
  #ax.scatter(puntLand['ball_y'], puntLand['kickLength'], color='red', alpha=0.6, edgecolors='white')
  # scatter plot touchdowns as big green dots with less transparency and larger white outline
  ax.scatter(puntDowned['ball_y'], puntDowned['kickLength'], color='blue', alpha=0.8, edgecolors='white', linewidth=2, s=125)
  # scatter plot touchdowns as big red dots with less transparency and larger white outline
  ax.scatter(fairCatch['ball_y'], fairCatch['kickLength'], color='yellow', alpha=0.8, edgecolors='white', linewidth=2, s=125)
  
  ax.scatter(touchback['ball_y'], touchback['kickLength'], color='purple', alpha=0.8, edgecolors='white', linewidth=2, s=125)
  
  ax.scatter(outOfBounds['ball_y'], outOfBounds['kickLength'], color='white', alpha=0.8, edgecolors='white', linewidth=2, s=125)

  ax.set_yticks(np.arange(20, 80, 10));
  ax.set_xticks(np.linspace(-53.3, 0, 5))
...
plot_passes(punter_names = 'Tommy Townsend')
plot_passes(punter_names = 'Bryan Anger')
plot_passes(punter_names = 'Michael Dickson')
plot_passes(punter_names = 'Sterling Hofrichter')
plot_passes(punter_names = 'Jamie Gillan')
plot_passes(punter_names = 'Sam Koch')
plot_passes(punter_names = 'Corey Bojorquez')
plot_passes(punter_names = 'Joseph Charlton')
plot_passes(punter_names = 'A.J. Cole')
plot_passes(punter_names = "Jack Fox")
plot_passes(punter_names = 'Logan Cooke')
plot_passes(punter_names = 'Britton Colquitt')
plot_passes(punter_names = 'Ty Long')
plot_passes(punter_names = 'Corey Bojorquez')
plot_passes(punter_names = 'Jack Fox')
...
def plot_punts(punter_names = 'Tommy Townsend'):
  ...
  player = ballpp_df.loc[(ballp_df['displayName'] == punter_names)] #
  ...
  # scatter plot completions as small green dots with some transparency
  ax.scatter(puntReceived['ball_y'], puntReceived['ball_x'], color='green', alpha=0.6, edgecolors='white')
  # scatter plot incompletions as small red dots with some transparency
  #ax.scatter(puntLand['ball_x'], puntLand['ball_y'], color='red', alpha=0.6, edgecolors='white')
  # scatter plot touchdowns as big green dots with less transparency and larger white outline
  ax.scatter(puntDowned['ball_y'], puntDowned['ball_x'], color='blue', alpha=0.8, edgecolors='white', linewidth=2, s=125)
  # scatter plot touchdowns as big red dots with less transparency and larger white outline
  ax.scatter(fairCatch['ball_y'], fairCatch['ball_x'], color='yellow', alpha=0.8, edgecolors='white', linewidth=2, s=125)

  ax.scatter(touchback['ball_y'], touchback['ball_x'], color='purple', alpha=0.8, edgecolors='white', linewidth=2, s=125)
  
  ax.scatter(outOfBounds['ball_y'], outOfBounds['ball_x'], color='white', alpha=0.8, edgecolors='white', linewidth=2, s=125)

  ax.set_yticks(np.arange(120, 30, 10)); #
  ax.set_xticks(np.linspace(0, -53.3, 4)) #
  ...
  # set the title of our plot (all caps)
  ax.set_title('\n'+ punter_names + ' Punt Locations ' + '\n', color='white', alpha=0.6, fontsize=18)
  ax.set(xlabel="Absolute Yard Line", ylabel = "Short Field Position")
  ...
  # set the legend off to the side
  ax.legend(['Punt Received', 'Punt Downed', 'Fair Catch', 'Touchback'], 
            ncol=1, loc='upper left', bbox_to_anchor=(1.05, 1), facecolor='#233746', prop={'size': 16})

plot_punts()

plot_punts(punter_names = 'A.J. Cole')
...
data_frames20 = [totalPunt20, notReturned20, returned20, puntLeft20, puntRight20,
                 ...
                 puntWI30020, puntWI40020, puntWI50020, accuracy20, puntInsideNums20]
...
rand = merged20['totalDirectional'] / puntsReturnedTotal
...
merged20['epaOppSub'] = -1 * ((merged20['puntWI5'] * -1.32) + (merged20['puntWI10'] * -1.02) + (merged20['puntWI20'] * -0.55) + (merged20['puntWI30'] * .15) + (merged20['puntWI40'] * .865) + (merged20['puntWI50'] * 1.275))
merged20['pinnedPlayValue'] = merged20['pinnedKL'] / (merged20['pinnedOT'] + merged20['pinnedHT'])
merged20['PlayValue'] = merged20['kickLength'] / (merged20['operationTime'] + merged20['hangTime'])
merged20['pinPlayValue'] = merged20['pinKL'] / (merged20['pinOT'] + merged20['pinHT'])
merged20['pinnedPlayValue'] = merged20['pinnedPlayValue'].fillna(8)
...
accSpider20_df = merged20[['percentMiss', 'percentDirectional', 'puntWI5', 'puntWI10', 'puntWI20','totalPlayValue', 'epaOppSub', 'percentNR']].reset_index()
accSpider20_df
accSpider20_df[['CR', 'NR', 'P5 Rating', 'P10 Rating',
                'P20 Rating', 'PPR', 'EPSR', 'RR']] = scaler.fit_transform(accSpider20_df[['percentMiss', 'percentDirectional', 'puntWI5', 'puntWI10', 'puntWI20',
                                                                                                                                  'totalPlayValue', 'epaOppSub', 'percentNR']])
accSpider20_df = accSpider20_df[['displayName', 'CR', 'NR', 'P5 Rating', 'P10 Rating',
                                 'P20 Rating', 'PPR', 'EPSR', 'RR']]

accSpider20_df

accuracyMetric = accSpider20_df[['displayName', 'P5 Rating', 'P10 Rating', 'P20 Rating', 'NR', 'CR']]
accuracyMetric['accTotal'] = accuracyMetric.sum(axis=1, numeric_only=True)
accuracyMetric = accuracyMetric[['displayName', 'accTotal']]

playMetric = accSpider20_df[['displayName', 'PPR', 'RR', 'EPSR']]
...
x = totalMetric['accTotal'].values
y = totalMetric['playTotal'].values
annotations = totalMetric['displayName'].unique()

fig, ax = plt.subplots(figsize=(15, 15))

ax.grid(alpha=0.5)
# plot a vertical and horixontal line to create separate quadrants
ax.vlines(2, 0, 3, color='green', alpha=0.7, lw=4, linestyles='dashed')
ax.hlines(1.5, 0, 5, color='green', alpha=0.7, lw=4, linestyles='dashed')
ax.set_ylim(0, 3)
ax.set_xlim(0, 4)
ax.set_xlabel('Accuracy Rating', fontsize=20)
ax.set_ylabel('Play Rating', fontsize=20)
...
ax.annotate('Good game play effect, good accuracy', xy=(x.max() - 1.8, y.max() + .1), **annot_styles)
ax.annotate('Good game play effect, bad accuracy', xy=(x.min() - .7, y.max() + .1), **annot_styles)
...
ax.set_title('Game Play Effect vs Accuracy Comparison 2020', fontsize=20);
...
for x in range(len(accSpider20_df['displayName'])):
    if accSpider20_df['displayName'][x] == 'Tommy Townsend':
...
title1_text = axs['title'].text(0.01, 0.65, 'Tommy Townsend', fontsize=25,
                                fontproperties=robotto_bold.prop, ha='left', va='center')
title2_text = axs['title'].text(0.01, 0.25, 'Kansas City Chiefs', fontsize=20,
                                fontproperties=robotto_regular.prop,
...
title4_text = axs['title'].text(0.99, 0.25, 'Punter Rating 2020', fontsize=20,
                                fontproperties=robotto_regular.prop,
                                ha='right', va='center', color='#B6282F')

playSpider20_df = merged20[['pinnedPlayValue', 'PlayValue', 'pinPlayValue', 'epaOppSub', 'percentNR']].reset_index()
playSpider20_df
